            ...
            if ((aFOEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_FOE)
            {
               switch (aFOEp->OpCode)
               {
                  ...
                  case ECT_FOE_ERROR:
                  {
					 ...
					 else if(aFOEp->ErrorCode == 0x8002)
					 {
						 wkc = -EC_ERR_TYPE_FOE_ACCESS_DENIED;
					 }
					 else if (aFOEp->ErrorCode == 0x8003)
					 {
						 wkc = -EC_ERR_TYPE_FOE_DISKFULL;
					 }
					 else if (aFOEp->ErrorCode == 0x8004)
					 {
						 wkc = -ECAT_FOE_ERRCODE_ILLEGAL;
					 }
					 else if (aFOEp->ErrorCode == 0x8005)
					 {
						 wkc = -ECAT_FOE_ERRCODE_PACKENO;
					 }
					 else if (aFOEp->ErrorCode == 0x8006)
					 {
						 wkc = -ECAT_FOE_ERRCODE_EXISTS;
					 }
					 else if (aFOEp->ErrorCode == 0x8007)
					 {
						 wkc = -ECAT_FOE_ERRCODE_NOUSER;
					 }
					 else if (aFOEp->ErrorCode == 0x8008)
					 {
						 wkc = -ECAT_FOE_ERRCODE_BOOTSTRAPONLY;
					 }
					 else if (aFOEp->ErrorCode == 0x8009)
					 {
						 wkc = -ECAT_FOE_ERRCODE_NOTINBOOTSTRAP;
					 }
					 else if (aFOEp->ErrorCode == 0x800A)
					 {
						 wkc = -ECAT_FOE_ERRCODE_NORIGHTS;
					 }
					 else if (aFOEp->ErrorCode == 0x800B)
					 {
						 wkc = -ECAT_FOE_ERRCODE_PROGERROR;
					 }
					 else if (aFOEp->ErrorCode == 0x800C)
					 {
						 wkc = -ECAT_FOE_ERRCODE_INVALID_CHECKSUM;
					 }
					 else if (aFOEp->ErrorCode == 0x800D)
					 {
						 wkc = -ECAT_FOE_ERRCODE_INVALID_FIRMWARE;
					 }
                     else
                     {
                        wkc = -EC_ERR_TYPE_FOE_ERROR;
